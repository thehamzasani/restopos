...
// prisma/seed.ts
// ✅ Supabase compatible — no Promise.all() used anywhere
...
async function main() {
  ...
  // ─── Menu Items ───────────────────────────────────────────────────────────
  // Sequential awaits — no Promise.all() to avoid PgBouncer prepared statement errors

  // Biryani
  const biryani1 = await prisma.menuItem.create({ data: { name: 'Chicken Biryani', description: 'Aromatic basmati rice with spiced chicken', price: 350, categoryId: biryani.id, isAvailable: true, preparationTime: 20 } })
  const biryani2 = await prisma.menuItem.create({ data: { name: 'Mutton Biryani', description: 'Slow-cooked mutton with fragrant rice', price: 550, categoryId: biryani.id, isAvailable: true, preparationTime: 25 } })
  const biryani3 = await prisma.menuItem.create({ data: { name: 'Beef Biryani', description: 'Tender beef with spiced basmati rice', price: 420, categoryId: biryani.id, isAvailable: true, preparationTime: 20 } })
  const biryani4 = await prisma.menuItem.create({ data: { name: 'Special Biryani', description: 'Mixed meat biryani with boiled egg', price: 650, categoryId: biryani.id, isAvailable: true, preparationTime: 25 } })

  // Karahi & Handi
  const karahi1 = await prisma.menuItem.create({ data: { name: 'Chicken Karahi', description: 'Tomato-based karahi with tender chicken', price: 850, categoryId: karahi.id, isAvailable: true, preparationTime: 30 } })
  const karahi2 = await prisma.menuItem.create({ data: { name: 'Mutton Karahi', description: 'Rich mutton karahi cooked in traditional style', price: 1400, categoryId: karahi.id, isAvailable: true, preparationTime: 40 } })
  const karahi3 = await prisma.menuItem.create({ data: { name: 'Beef Karahi', description: 'Spicy beef karahi with fresh ginger', price: 1100, categoryId: karahi.id, isAvailable: true, preparationTime: 35 } })
  const karahi4 = await prisma.menuItem.create({ data: { name: 'Chicken Handi', description: 'Creamy white handi with tender chicken', price: 950, categoryId: karahi.id, isAvailable: true, preparationTime: 30 } })
  const karahi5 = await prisma.menuItem.create({ data: { name: 'Daal Mash', description: 'Slow-cooked white lentils with butter', price: 350, categoryId: karahi.id, isAvailable: true, preparationTime: 20 } })

  // BBQ & Tikka
  const bbq1 = await prisma.menuItem.create({ data: { name: 'Chicken Tikka (6pcs)', description: 'Marinated chicken tikka grilled on charcoal', price: 650, categoryId: bbq.id, isAvailable: true, preparationTime: 25 } })
  const bbq2 = await prisma.menuItem.create({ data: { name: 'Seekh Kebab (4pcs)', description: 'Minced beef kebabs with spices', price: 480, categoryId: bbq.id, isAvailable: true, preparationTime: 20 } })
  const bbq3 = await prisma.menuItem.create({ data: { name: 'Boti Kebab (6pcs)', description: 'Tender mutton boti grilled to perfection', price: 750, categoryId: bbq.id, isAvailable: true, preparationTime: 25 } })
  const bbq4 = await prisma.menuItem.create({ data: { name: 'Chapli Kebab (2pcs)', description: 'Peshwari-style flat beef kebabs', price: 380, categoryId: bbq.id, isAvailable: true, preparationTime: 15 } })
  const bbq5 = await prisma.menuItem.create({ data: { name: 'Mix BBQ Platter', description: 'Assorted BBQ platter for 2', price: 1500, categoryId: bbq.id, isAvailable: true, preparationTime: 35 } })

  // Fast Food
  const ff1 = await prisma.menuItem.create({ data: { name: 'Zinger Burger', description: 'Crispy chicken fillet burger with coleslaw', price: 450, categoryId: fastFood.id, isAvailable: true, preparationTime: 15 } })
  const ff2 = await prisma.menuItem.create({ data: { name: 'Beef Burger', description: 'Juicy beef patty with fresh veggies', price: 380, categoryId: fastFood.id, isAvailable: true, preparationTime: 12 } })
  const ff3 = await prisma.menuItem.create({ data: { name: 'Club Sandwich', description: 'Triple decker sandwich with chicken', price: 420, categoryId: fastFood.id, isAvailable: true, preparationTime: 10 } })
  const ff4 = await prisma.menuItem.create({ data: { name: 'Chicken Shawarma', description: 'Grilled chicken wrap with garlic sauce', price: 280, categoryId: fastFood.id, isAvailable: true, preparationTime: 10 } })
  const ff5 = await prisma.menuItem.create({ data: { name: 'French Fries (Large)', description: 'Crispy golden french fries', price: 220, categoryId: fastFood.id, isAvailable: true, preparationTime: 8 } })
  const ff6 = await prisma.menuItem.create({ data: { name: 'Chicken Nuggets (8pcs)', description: 'Golden fried chicken nuggets', price: 320, categoryId: fastFood.id, isAvailable: true, preparationTime: 10 } })

  // Drinks
  const drink1 = await prisma.menuItem.create({ data: { name: 'Coca Cola', description: '330ml chilled can', price: 80, categoryId: drinks.id, isAvailable: true, preparationTime: 2 } })
  const drink2 = await prisma.menuItem.create({ data: { name: 'Mango Shake', description: 'Fresh mango blended with milk', price: 220, categoryId: drinks.id, isAvailable: true, preparationTime: 5 } })
  const drink3 = await prisma.menuItem.create({ data: { name: 'Strawberry Shake', description: 'Creamy strawberry milkshake', price: 220, categoryId: drinks.id, isAvailable: true, preparationTime: 5 } })
  const drink4 = await prisma.menuItem.create({ data: { name: 'Lassi (Sweet)', description: 'Traditional Pakistani sweet yogurt drink', price: 180, categoryId: drinks.id, isAvailable: true, preparationTime: 5 } })
  const drink5 = await prisma.menuItem.create({ data: { name: 'Fresh Lime Soda', description: 'Refreshing lime with soda', price: 150, categoryId: drinks.id, isAvailable: true, preparationTime: 3 } })
  const drink6 = await prisma.menuItem.create({ data: { name: 'Mineral Water', description: '500ml water bottle', price: 60, categoryId: drinks.id, isAvailable: true, preparationTime: 1 } })

  // Desserts
  const dessert1 = await prisma.menuItem.create({ data: { name: 'Gulab Jamun (4pcs)', description: 'Soft milk solids soaked in sugar syrup', price: 180, categoryId: desserts.id, isAvailable: true, preparationTime: 5 } })
  const dessert2 = await prisma.menuItem.create({ data: { name: 'Kheer', description: 'Creamy rice pudding with cardamom', price: 200, categoryId: desserts.id, isAvailable: true, preparationTime: 5 } })
  const dessert3 = await prisma.menuItem.create({ data: { name: 'Gajar Halwa', description: 'Carrot halwa with dry fruits', price: 220, categoryId: desserts.id, isAvailable: true, preparationTime: 5 } })
  const dessert4 = await prisma.menuItem.create({ data: { name: 'Ice Cream (2 Scoops)', description: 'Choice of vanilla, chocolate or strawberry', price: 200, categoryId: desserts.id, isAvailable: true, preparationTime: 3 } })

  // Naan & Roti
  const bread1 = await prisma.menuItem.create({ data: { name: 'Tandoori Naan', description: 'Fresh naan from the tandoor', price: 60, categoryId: naanRoti.id, isAvailable: true, preparationTime: 5 } })
  const bread2 = await prisma.menuItem.create({ data: { name: 'Butter Naan', description: 'Naan brushed with butter', price: 80, categoryId: naanRoti.id, isAvailable: true, preparationTime: 5 } })
  const bread3 = await prisma.menuItem.create({ data: { name: 'Roghni Naan', description: 'Sesame topped naan', price: 100, categoryId: naanRoti.id, isAvailable: true, preparationTime: 5 } })
  const bread4 = await prisma.menuItem.create({ data: { name: 'Chapati', description: 'Whole wheat flatbread', price: 30, categoryId: naanRoti.id, isAvailable: true, preparationTime: 3 } })
  const bread5 = await prisma.menuItem.create({ data: { name: 'Paratha', description: 'Layered flaky flatbread', price: 70, categoryId: naanRoti.id, isAvailable: true, preparationTime: 5 } })
  ...
  // ─── Seed Orders (Last 30 days) ───────────────────────────────────────────
  // All sequential — no Promise.all() to avoid PgBouncer issues

  const tables = await prisma.table.findMany()

  const allMenuItems = [
    biryani1, biryani2, biryani3, biryani4,
    karahi1, karahi2, karahi3, karahi4, karahi5,
    bbq1, bbq2, bbq3, bbq4, bbq5,
    ff1, ff2, ff3, ff4, ff5, ff6,
    drink1, drink2, drink3, drink4, drink5, drink6,
    dessert1, dessert2, dessert3, dessert4,
    bread1, bread2, bread3, bread4, bread5,
  ]
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    // Fri/Sat are busiest days in Pakistan
    const dayOfWeek = baseDate.getDay()
    const isWeekend = dayOfWeek === 5 || dayOfWeek === 6
    const ordersPerDay = isWeekend
      ? Math.floor(Math.random() * 4) + 5   // 5-8 orders
      : Math.floor(Math.random() * 3) + 3   // 3-5 orders

    for (let j = 0; j < ordersPerDay; j++) {
      // Random time 11am–11pm
      const hour   = Math.floor(Math.random() * 12) + 11
      const minute = Math.floor(Math.random() * 60)
      ...
      // Pick 1–4 random items with quantities
      const itemCount = Math.floor(Math.random() * 4) + 1
      const selectedItems: { item: typeof allMenuItems[0]; quantity: number }[] = []
      for (let k = 0; k < itemCount; k++) {
        const item     = allMenuItems[Math.floor(Math.random() * allMenuItems.length)]
        const quantity = Math.floor(Math.random() * 2) + 1
        selectedItems.push({ item, quantity })
      ...
      // Create order sequentially
      await prisma.order.create({
        data: {
          orderNumber:     `ORD-${orderDate.getTime()}-${Math.floor(Math.random() * 9999).toString().padStart(4, '0')}`,
          orderType,
          customerName:    orderType !== 'DINE_IN' ? customerName : null,
          customerPhone:   orderType !== 'DINE_IN' ? `+923${Math.floor(Math.random() * 900000000 + 100000000)}` : null,
          tableId:         orderType === 'DINE_IN'  ? tables[Math.floor(Math.random() * tables.length)].id : null,
          deliveryAddress: orderType === 'DELIVERY' ? deliveryAddresses[Math.floor(Math.random() * deliveryAddresses.length)] : null,
          deliveryNote:    orderType === 'DELIVERY' ? 'Please call on arrival' : null,
          deliveryFee,
          subtotal,
          tax,
          discount: 0,
  ...
  console.log(`✅ Created ${orderCount} sample orders (last 30 days)`)
  ...
  console.log('')
  console.log('📋 Summary:')
  console.log('   - 4 Users (admin, manager, cashier, kitchen)')
  console.log('   - 7 Pakistani food categories')
  console.log('   - 34 Menu items with PKR pricing')
  ...
  console.log('   Admin:   admin@restopos.com   / admin123')
  console.log('   Manager: manager@restopos.com / admin123')
  console.log('   Cashier: cashier@restopos.com / admin123')
  console.log('   Kitchen: kitchen@restopos.com / admin123')
}
